...
class Crypt_RSA_Math_GMP
{
    ...
    function bitLen($num)
    {
        ...
        $bit_len = bin_strlen($tmp) * 8;
        $tmp = ord($tmp{bin_strlen($tmp) - 1});
        if (!$tmp) {
            $bit_len -= 8;
        }
        else {
    ...
    function bitOr($num1, $num2, $start_pos)
    {
        ...
        if ($start_byte < bin_strlen($tmp1)) {
            $tmp2 |= bin_substr($tmp1, $start_byte);
            $tmp1 = bin_substr($tmp1, 0, $start_byte) . $tmp2;
        }
        else {
    ...
    function subint($num, $start, $length)
    {
        ...
        $tmp = bin_substr($this->int2bin($num), $start_byte, $byte_length);
